...
async function fetchData() {
    try {
        ...
        console.log("LIST::: ", productArray);
        return productArray;
        // console.log('Product List: ' , allProductList); // This will log the product list obtained from the API
    } catch (error) {
        console.error("Error fetching product data:", error);
    }
}
console.log("Product: ", productArray); // This will log the product list obtained from the API
...
const FarzaaContextProvider = ({ children }) => {
    ...
    // console.log("QWEQWE: " , productArray);
    ...
    const handleCategoryFilter = (category) => {
        if (category === null) {
            setFilteredProducts(productArray); // Show all products
        } else {
    ...
    useEffect(() => {
        if (selectedTags.length > 0) {
            const filteredByTags = productArray.filter((product) =>
                selectedTags.includes(product.category)
            );
    ...
    // Define fetchPaginatedProducts function to fetch paginated products
    ...
    useEffect(() => {
        // Fetch paginated products when component mounts
    ...
    const addToCart = (itemId) => {
        ...
        const itemToAdd = productArray.find((item) => item.id === itemId);
    ...
    const addWishlistToCart = () => {
        ...
        wishlist.forEach((wishlistItem) => {
            ...
            if (existingCartItemIndex !== -1) {
                ...
                updatedCartItems[existingCartItemIndex].total +=
                    wishlistItem.price;
            } else {
    ...
    const handleRemoveJeweleryItemWishlist = (itemId) => {
        const updatedItems = jeweleryWishlist.filter(
            (item) => item.id !== itemId
        );
    ...
    const handleRemoveJeweleryCartItem = (itemId) => {
        const updatedItems = jeweleryAddToCart.filter(
            (item) => item.id !== itemId
        );
        ...
        if (itemToAdd) {
            ...
            if (!jeweleryAddToCart.some((item) => item.id === itemId)) {
                ...
                updatedAddToCartItems[existingItemIndex].total =
                    updatedAddToCartItems[existingItemIndex].quantity *
                    itemToAdd.price;
...
const useStateContext = () => useContext(FarzaaContext);

export { FarzaaContext, FarzaaContextProvider, useStateContext };
